...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"**🚫 [AlessandrO](t.me/Boicottato) non è al momento su Telegram da:** `{total_afk_time}`\nAttendi il ritorno del mio padrone per avere una risposta." + \
            ...
            if reason \
            else f"**🚫 [AlessandrO](t.me/Boicottato) non è al momento su Telegram da:** `{total_afk_time}`\nAttendi il ritorno del mio padrone per avere una risposta." + \
            f"\n**🔎 Motivo assenza:** {reason}\n\n🖕🏻 Questo userbot è hostato su un server privato pertanto spammare non aumenterà i consumi, HEROKU merda."
